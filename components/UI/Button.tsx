...
export const Button: React.FC<ButtonProps> = ({ variant = 'primary', children, icon, className, ...props }) => {
  ...
  const baseStyles = "relative inline-flex items-center justify-center px-8 py-4 text-sm font-medium tracking-widest uppercase transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] rounded-full font-body group overflow-hidden cursor-pointer active:scale-95";
  ...
  return (
    <motion.button
      ...
      animate={reducedMotion ? { x: 0, y: 0 } : { x: position.x, y: position.y }}
      transition={
        reducedMotion
          ? { duration: 0 }
    ...
    >
      ...
      {/* Cinematic Shine Effect - Primary Only (disabled under reduced motion) */}
