export type ProjectKind = 'live' | 'concept' | 'platform';
...
export interface Project {
  ...
  description?: string;
  status?: string;
  /** Derived from existing status labels without inventing client claims. */
  kind?: ProjectKind;
  problem?: string;
  solution?: string;
  outcome?: string;
...
export const HERO_FEATURED =
  PROJECTS.find((project) => project.id === "global-jet-concierge") ?? PROJECTS[0];
